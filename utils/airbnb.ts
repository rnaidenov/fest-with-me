...
const generateUrl = (currency, numPeople, checkInDate, checkOutDate, city, country, roomType = '') =>
...
const query = (currency, numPeople, checkInDate, checkOutDate, city, country) => async (roomType = '') => {
  // TODO: check in and out data to be of string type 
  // TODO: Node cache

  const url = generateUrl(currency, numPeople, checkInDate, checkOutDate, city, country, roomType);
  ...
  const firstFilterV2 = sections.find(section => section.sectionComponentType === FILTER_COMPONENT_TYPE)
  const avgPriceText = firstFilterV2.section.discreteFilterItems[0].text.subtitle;
...
const queryAirbnb = async (currency, numPeople, checkInDate, checkOutDate, city, country) => {
  const queryForProperty = query(currency, numPeople, checkInDate, checkOutDate, city, country);

  const avgPriceTotal = await queryForProperty();
  const avgPricePrivateRoom = await queryForProperty(AccommodationType.PrivateRoom);
  const avgPriceSharedRoom = await queryForProperty(AccommodationType.SharedRoom);
  const avgPriceEntirePlace = await queryForProperty(AccommodationType.EntireHome);
...
export const handler = async (req: Request, _ctx: HandlerContext): Response => {
  try {
    ...
    // TODO: Validation!
    const { currency, numPeople, checkInDate, checkOutDate, city, country } = body
    ...
    const data = await queryAirbnb(currency, numPeople, checkInDate, checkOutDate, city, country);
